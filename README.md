...
Leak Guard is a ready-to-use template for stopping secrets from entering source control. It provides three guardrails:

1. **Pre-commit** — fast scan of staged changes on developer laptops  
2. **Pre-push** — full working-tree scan on developer laptops  
3. **Pull Request scan (GitHub Actions)** — checks PRs, uploads a JSON report, and opens an Issue when a leak is found
...
3. Run the developer setup below.
...
> If `pre-commit` is not available, install it with `pipx install pre-commit` or `pip install --user pre-commit`.
...
1. **Allow-list safe folders** — add paths to `[allowlist].paths` for tutorials, examples and fixtures.  
2. **Tighten noisy rules** — if a rule matches harmless text, add context words or narrow the pattern.  
3. **Use `.gitleaksignore` sparingly** — record the file and a brief reason. Keep this short to avoid masking real problems.  
4. **Review monthly** — tweak rules and keep a short changelog in your security notes.
...
The pull request Issue that is opened on failure gives useful timestamps to copy across.
...
1. **Remove the secret from code** — replace with configuration that reads from an environment variable or a managed secret store.  
2. **Rotate or revoke at the source** — follow `docs/REVOCATION-PLAYBOOK.md` for common services.  
3. **Check logs around the window of exposure** — look for unusual use.  
4. **Record detection and fix times** — update the auto-opened Issue and `docs/MTTR-metrics.md`.
